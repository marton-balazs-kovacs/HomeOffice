...
The number of trials where the efficiency rating is missing is `r filter(processed, is.na(now_before_eff)) %>% count() %>% pull(n)`. It is because this question was not shown to those who said that they are not working more from home. This number also includes a case, where the response was dropped because the respondent stated in the comment that he did not understand the question.
